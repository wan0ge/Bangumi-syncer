...
def test_other_accounts_archive_uses_archive_total_eps():
    """多账号下其余账号归档同样以 Archive 补全的 eps 为总集数。"""
    ...
    with patch("app.services.sync_service.config_manager") as mock_cfg:
        ...
        with patch.object(
            ...
            return_value=[("bangumi", primary), ("bangumi-2", other)],
        ):
            with patch.object(service, "_retry_mark_episode", return_value=None):
                service._mark_episode_for_other_accounts(
                    item, primary, "123", "456", "TV Anime X"
                )
